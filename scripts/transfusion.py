...
try:
    from transfusion.constants import DEFAULT_IMAGE_TOKEN, IMAGE_TOKEN_INDEX
    from transfusion.conversation import conv_templates
    from transfusion.mm_utils import (
        get_model_name_from_path,
        process_images,
        tokenizer_image_token,
    )
    from transfusion.model.builder import load_pretrained_model
except Exception as e:
    eval_logger.debug("Transfusion is not installed. Please install Transfusion to use this model.\nError: %s" % e)
...
@register_model("transfusion")
class Transfusion(lmms):
    """
    Transfusion Model
    """
    ...
    def __init__(
        self,
        pretrained: str = "lavinal712/transfusion-7b",
        ...
        **kwargs,
        ...
        transfusion_model_args = {
            "multimodal": True,
        }
        if customized_config is not None:
            transfusion_model_args["customized_config"] = customized_config
        if attn_implementation is not None:
            transfusion_model_args["attn_implementation"] = attn_implementation
        if "use_flash_attention_2" in kwargs:
            transfusion_model_args["use_flash_attention_2"] = kwargs["use_flash_attention_2"]
        model_name = model_name if model_name is not None else get_model_name_from_path(pretrained)
        try:
            # Try to load the model with the multimodal argument
            self._tokenizer, self._model, self._image_processor, self._max_length = load_pretrained_model(pretrained, None, model_name, device_map=self.device_map, **transfusion_model_args)
        except TypeError:
            # for older versions of LLaVA that don't have multimodal argument
            transfusion_model_args.pop("multimodal", None)
            self._tokenizer, self._model, self._image_processor, self._max_length = load_pretrained_model(pretrained, None, model_name, device_map=self.device_map, **transfusion_model_args)
    ...
    def generate_until_multi_round(self, requests) -> List[str]:
        raise NotImplementedError("TODO: Implement multi-round generation for Transfusion")
